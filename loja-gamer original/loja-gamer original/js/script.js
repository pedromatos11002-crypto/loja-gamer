...
function pegarMeusPedidos() {
  let todosPedidos = JSON.parse(localStorage.getItem("todosPedidos")) || [];
  return todosPedidos.filter(pedido => pedido.email === usuarioLogado.email);
}
...
function limparPedidos() {
  if (!confirm("Deseja apagar seus pedidos?")) return;

  let todosPedidos = JSON.parse(localStorage.getItem("todosPedidos")) || [];

  todosPedidos = todosPedidos.filter(pedido => pedido.email !== usuarioLogado.email);
...
function renderizarPedidos() {
  ...
  const pedidos = pegarMeusPedidos();
